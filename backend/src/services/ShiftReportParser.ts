...
import {
    ...
    DepartmentSale,
    ItemSale,
    ShiftReportException as ExceptionType,
} from '../types/shiftReportExtract.types';
...
export class ShiftReportParser {
     ...
     */
    static parse(rawText: string): Partial<ShiftReportExtract> {
        Logger.info('Parsing OCR text with deterministic parser...');

        const result: Partial<ShiftReportExtract> = {
            rawText,
    ...
    private static parseStoreMetadata(text: string) {
        const meta: any = {};

        // Register ID
        const registerMatch = text.match(/register[:\s#]*(\w+)/i);
        if (registerMatch) meta.registerId = registerMatch[1];

        // Operator ID
        const operatorMatch = text.match(/(?:operator|cashier|clerk)[:\s#]*(\w+)/i);
        if (operatorMatch) meta.operatorId = operatorMatch[1];

        // Till ID
        const tillMatch = text.match(/till[:\s#]*(\w+)/i);
        if (tillMatch) meta.tillId = tillMatch[1];
    ...
    private static parseBalances(text: string) {
        const balances: any = {};

        // Beginning balance
        const beginMatch = text.match(/(?:beginning|opening|start)\s*(?:balance|drawer)[:\s]*\$?([\d,]+\.?\d*)/i);
        if (beginMatch) balances.beginningBalance = this.parseNumber(beginMatch[1]);

        // Ending balance
        const endMatch = text.match(/(?:ending|closing|end)\s*(?:balance|drawer)[:\s]*\$?([\d,]+\.?\d*)/i);
        if (endMatch) balances.endingBalance = this.parseNumber(endMatch[1]);

        // Accountability
        const accountMatch = text.match(/(?:accountability|accountable)[:\s]*\$?([\d,]+\.?\d*)/i);
        if (accountMatch) balances.closingAccountability = this.parseNumber(accountMatch[1]);

        // Cashier counted
        const countedMatch = text.match(/(?:counted|actual|count)[:\s]*\$?([\d,]+\.?\d*)/i);
        if (countedMatch) balances.cashierCounted = this.parseNumber(countedMatch[1]);

        // Over/Short variance
        const varianceMatch = text.match(/(?:over|short|variance|o\/s)[:\s]*[\(\$]*([\-\d,]+\.?\d*)/i);
        if (varianceMatch) balances.cashVariance = this.parseNumber(varianceMatch[1]);
    ...
    private static parseSalesSummary(text: string) {
        const sales: any = {};

        // Gross sales
        const grossMatch = text.match(/gross\s*sales[:\s]*\$?([\d,]+\.?\d*)/i);
        if (grossMatch) sales.grossSales = this.parseNumber(grossMatch[1]);

        // Net sales
        const netMatch = text.match(/net\s*sales[:\s]*\$?([\d,]+\.?\d*)/i);
        if (netMatch) sales.netSales = this.parseNumber(netMatch[1]);

        // Total sales (fallback)
        if (!sales.grossSales && !sales.netSales) {
            const totalMatch = text.match(/total\s*sales[:\s]*\$?([\d,]+\.?\d*)/i);
            if (totalMatch) sales.grossSales = this.parseNumber(totalMatch[1]);
        }

        // Refunds
        const refundMatch = text.match(/refunds?[:\s]*\$?([\d,]+\.?\d*)/i);
        if (refundMatch) sales.refunds = this.parseNumber(refundMatch[1]);

        // Discounts
        const discountMatch = text.match(/discounts?[:\s]*\$?([\d,]+\.?\d*)/i);
        if (discountMatch) sales.discounts = this.parseNumber(discountMatch[1]);

        // Tax
        const taxMatch = text.match(/(?:tax|taxes)[:\s]*\$?([\d,]+\.?\d*)/i);
        if (taxMatch) sales.taxTotal = this.parseNumber(taxMatch[1]);

        // Transactions
        const transMatch = text.match(/(?:transactions?|trans|customers?)[:\s]*(\d+)/i);
        if (transMatch) sales.totalTransactions = parseInt(transMatch[1]);
    ...
    private static parseFuel(text: string) {
        const fuel: any = {};

        // Fuel sales
        const fuelMatch = text.match(/fuel\s*(?:sales)?[:\s]*\$?([\d,]+\.?\d*)/i);
        if (fuelMatch) fuel.fuelSales = this.parseNumber(fuelMatch[1]);

        // Fuel gross
        const fuelGrossMatch = text.match(/fuel\s*gross[:\s]*\$?([\d,]+\.?\d*)/i);
        if (fuelGrossMatch) fuel.fuelGross = this.parseNumber(fuelGrossMatch[1]);

        // Gallons
        const gallonsMatch = text.match(/(?:gallons?|gal)[:\s]*([\d,]+\.?\d*)/i);
        if (gallonsMatch) fuel.fuelGallons = this.parseNumber(gallonsMatch[1]);
    ...
    private static parseInsideSales(text: string) {
        const inside: any = {};

        // Inside sales
        const insideMatch = text.match(/inside\s*(?:sales)?[:\s]*\$?([\d,]+\.?\d*)/i);
        if (insideMatch) inside.insideSales = this.parseNumber(insideMatch[1]);

        // Merchandise
        const merchMatch = text.match(/merchandise[:\s]*\$?([\d,]+\.?\d*)/i);
        if (merchMatch) inside.merchandiseSales = this.parseNumber(merchMatch[1]);

        // Prepays
        const prepayMatch = text.match(/prepay[s\s]*(?:initiated)?[:\s]*\$?([\d,]+\.?\d*)/i);
        if (prepayMatch) inside.prepaysInitiated = this.parseNumber(prepayMatch[1]);
    ...
    private static parseTenders(text: string) {
        const tenders: any = {};
        ...
        if (totalMatch) tenders.totalTenders = this.parseNumber(totalMatch[1]);
    ...
    private static parseSafeActivity(text: string) {
        const safe: any = {};
    ...
    private static parseDepartmentSales(text: string): DepartmentSale[] {
        ...
        while ((match = lineRegex.exec(deptSection[0])) !== null) {
            ...
            if (['total', 'net', 'gross', 'tax', 'cash', 'credit'].some(w => name.toLowerCase().includes(w))) continue;
    ...
    private static parseExceptions(text: string): ExceptionType[] {
    ...
    private static calculateConfidence(result: Partial<ShiftReportExtract>): number {
        let score = 0;
        let total = 0;

        if (result.salesSummary) { score += result.salesSummary.confidence || 0.5; total++; }
        if (result.balances) { score += result.balances.confidence || 0.5; total++; }
        if (result.fuel) { score += result.fuel.confidence || 0.5; total++; }
        if (result.tenders) { score += result.tenders.confidence || 0.5; total++; }
        if (result.departmentSales && result.departmentSales.length > 0) { score += 0.6; total++; }
