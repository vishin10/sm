import { Shift, Alert, Insight } from '@prisma/client';
import { prisma } from '../config/database';

export class AnalyticsService {
    static async detectAnomalies(shift: any): Promise<any[]> {
        const alerts: any[] = []; // Alert create inputs
    ...
    static async generateInsights(storeId: string): Promise<any[]> {
        const insights: any[] = [];
        ...
        if (shifts.length === 0) return insights;
        ...
        // Fuel/Inside Ratio
        const totalFuel = shifts.reduce((sum: number, s: any) => sum + Number(s.fuelSales), 0);
        const totalInside = shifts.reduce((sum: number, s: any) => sum + Number(s.nonFuelSales), 0);
        ...
        // Avg Cash Variance
        const avgVariance = shifts.reduce((sum: number, s: any) => sum + Number(s.cashVariance), 0) / shifts.length;
        insights.push({
            title: 'Cash Accuracy',
    ...
    static async getTrends(storeId: string) {
        const sevenDaysAgo = new Date();
        sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
